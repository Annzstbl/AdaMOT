...
def plot_tracking(image, tlwhs, obj_ids, scores=None, pt_inds=None, frame_id=0, fps=0., ids2=None):
    im = np.ascontiguousarray(np.copy(image))
    im_h, im_w = im.shape[:2]

    top_view = np.zeros([im_w, im_w, 3], dtype=np.uint8) + 255

    text_scale = max(1, image.shape[1] / 1400.)
    text_thickness = 2
    ...
    cv2.rectangle(im, (0, 0), (200, 55), (0, 255, 255), -1, 1)
    cv2.putText(im, 'Frame:%d' % (frame_id),
                (15, 38), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (0, 0, 0), thickness=4)
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        cv2.putText(im, id_text, (intbox[0], intbox[1] + 30), cv2.FONT_HERSHEY_TRIPLEX, text_scale, (0, 0, 255),
                    thickness=text_thickness)
    return im
